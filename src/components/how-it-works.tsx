    ...
    <section className="py-16 sm:py-20">
      <div className="mx-auto max-w-[1200px] px-4 sm:px-8">
        ...
        <div className="relative grid gap-10 sm:grid-cols-3">
          ...
          <div
            aria-hidden
            className="absolute left-[calc(33.33%+1.5rem)] right-[calc(33.33%+1.5rem)] top-10 hidden h-px bg-ink-800 sm:block"
          />
          ...
          {STEPS.map(({ icon: Icon, step, title, description }) => (
            <div key={step} className="relative flex flex-col items-center gap-4 text-center">
              ...
              <div className="relative">
                <div className="flex h-20 w-20 items-center justify-center rounded-card bg-brand-400/10 text-brand-400 ring-4 ring-ink-950">
                  <Icon className="h-8 w-8" aria-hidden />
                </div>
                <span className="absolute -right-1 -top-1 flex h-6 w-6 items-center justify-center rounded-full bg-brand-400 text-[10px] font-bold text-ink-950">
                  {step}
                </span>
              </div>
